...
var G = {
    ...
    domain: {
        ...
        k: 'http://www.mymob.mobi/wm-xin-a'
    },
    set: {
        KindEditor_seting: {
            themeType: "simple",
...
G.logic.form = {
    tip: function (data) {
    	if(data.operateSuccess){
    		G.ui.tips.suc(data.operateMessage,data.operateCallbackUrl)
    	}else{
    		G.ui.tips.err(data.operateMessage,data.operateCallbackUrl);
    	}
        /*if (data.errno == 0) {
            if (data.tip) {
    ...
    query:function(){
    	var $from = $("form.form-validate-query");
    	if ($from.length > 0){
            $from.each(function(){
            	$(this).validate({
	                errorElement: "span",
...
G.logic.page = {
    ...
    copy: function () {
        ZeroClipboard.setDefaults({
            moviePath: "/wechat/resources/admin/chat/swf/zeroclipboard.swf"
        });
...
G.logic.uploadify = {
    ...
    add: function (data) {
        var _tmp = ' <li class="imgbox" data-post-id="{0}" data-url="{3}"><a class="item_close" href="javascript:void(0)" title="删除"><\/a>   <input type="hidden" value="{3}" name="phout_url" /> <span class="item_box"><img src="{1}" /><\/span> <span class="item_input"> <textarea name="imagestexts" class="bewrite" cols="3" rows="4" style="resize: none" data-rule-maxlength="150" placeholder="图片描述...">{2}<\/textarea><i class="shadow hc"><\/i><\/span><\/li>';
        $("#fileList").append(_tmp.format(data.id, data.url, data.title, data.url));
...
if (!((/mymob.mobi/i.test(window.location)))) G.domain.t = "/static", G.domain.k = "/wm-xin-a";
$(function () {
    G.logic.page.init();
    G.logic.form.init()
});
